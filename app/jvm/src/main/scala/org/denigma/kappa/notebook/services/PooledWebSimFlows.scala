...
trait PooledWebSimFlows extends WebSimFlows  {
  ...
  implicit protected def context: ExecutionContextExecutor

  def unmarshalFlow[T](implicit um: Unmarshaller[HttpResponse, T]): Flow[TryRequest, Future[T], NotUsed] = Flow[ TryRequest ].map{
  ...
  protected val simulationStatusFutureFlow: Flow[Int, Future[(TokenPoolMessage, WebSim.SimulationStatus)], NotUsed] = simulationStatusRequestFlow.via(tokenPool).map{
    case (Success(res), mess: TokenPoolMessage) =>  Unmarshal(res).to[WebSim.SimulationStatus].map(mess -> _)
  ...
  def modelResultsFlow(parallelism: Int, streamInterval: FiniteDuration): Flow[RunModel, (TokenPoolMessage, SimulationStatus), NotUsed] = runModelRequestFlow.via(timePool)
    .via(unmarshalFlow[Token]).mapAsync(parallelism)(identity(_)).via(tokenResultsFlow(parallelism, streamInterval))

  def tokenResultsFlow(parallelism: Int, streamInterval: FiniteDuration): Flow[Token, (TokenPoolMessage, SimulationStatus), NotUsed] =  Flow[Token].flatMapMerge(parallelism, {
      case token =>
        val source = Source.tick(0 millis, streamInterval, token)
        source.via( simulationStatusFutureFlow.mapAsync(1)(identity(_)))
          .upTo{ case (t, sim) => sim.percentage >= 100.0 || !sim.is_running.getOrElse(false)}
  })
}
