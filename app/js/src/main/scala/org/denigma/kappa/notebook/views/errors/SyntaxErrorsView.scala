...
class SyntaxErrorsView(val elem: Element,
  ...
  override def newItemView(item: Item): ItemView = this.constructItemView(item){
    case (el, _)=>
      ...
      new WebSimErrorView(el, input, item._1, item._2, errorCode, movements).withBinder(v=>new CodeBinder(v))
  }
...
class WebSimErrorView(val elem: Element, input: Var[KappaMessage], file: KappaSourceFile, error: WebSimError, textUnderError: String, movements: Movements) extends BindableView {
