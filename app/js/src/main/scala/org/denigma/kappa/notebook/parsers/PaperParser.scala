...
case class PaperSelection(paper: IRI, page: Int, from_chunk: Int, to_chunk: Int, fromTokenOpt: Option[Int] = None, toTokenOpt: Option[Int] = None)
...
class PaperParser extends RDFParser with BasicParser {
  ...
  lazy val paper = P(PNAME_NS ~ ("in_paper" | "paper") ~ spaces ~ IRI).map(AST.IRI(_))
  ...
  lazy val d = P(optSpaces ~ ";" ~ optSpaces)
